...
**Migration repair:** if deployment stopped at `0007` with `incomplete input: SQLITE_ERROR`, pull the latest commit and rerun `npm run cloudflare:deploy`. The series trigger now avoids ambiguous nested `CASE` boundaries. Deployment first checks every migration in a disposable local database using the installed Wrangler SQL splitter; `npm run db:check` runs this check alone. Keep the existing database and PINs. See [deployment recovery](web/README.md#recover-from-the-0007-migration-error).
