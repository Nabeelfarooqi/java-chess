...
The latest web update adds measured connection ping, frame-scheduled dragging, short move/castling animations, larger mobile portraits, and Characters/Classic/Slate board styles. Finished games show both rivals, the updated pair record, and a same-clock rematch button. Character delivery images are 83% smaller, and healthy live connections use five-second recovery polls with separate clock-expiry checks. See [smoother play and appearance](web/README.md#smoother-play-and-appearance) for behavior and measurement limits.
