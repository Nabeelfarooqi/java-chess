...
## Watch while signed in

Click **Watch games** in the room header to watch your rivals using your normal player session. Select a live game to see its board, clocks, captured material, and final result. **Back to playing** closes the view without logging you out or changing your PIN identity. The button is unavailable during your own pending challenge or active game; a new personal challenge automatically closes watching so you can respond.

This uses the authenticated, read-only `GET /api/room?watch=1` endpoint (optional `&game=ID`). It excludes your own games and all pending challenges, grants no rights to move or alter another player’s game, and does not expose PINs, session hashes, exports, or pair records. It works even when guest spectator access is disabled. Closing the view cancels its requests; hidden tabs pause polling. No new migration, PIN setup, service, or hosting cost is required.
...
- **Capture premoves:** target an enemy piece, an empty square where you expect a pawn capture, or a friendly piece’s square to prepare a recapture. Both tap and drag work. If the opponent does not make your intended move legal, the queue cancels; a friendly piece is never captured. En passant and promotion captures use normal chess rules.
