...
export async function prepareAppIcons(){
    ...
    await Promise.all([180,192,512].map(async size=>{
        ...
        await sharp({create:{width:size,height:size,channels:4,background:'#23362b'}}).composite([{input:knight,left:inset,top:inset}]).png().toFile(fileURLToPath(new URL(`${size}.png`,directory)));
    }));
}
