...
- An actual blocked optional Club JavaScript chunk, a contained error boundary, continued room access, and recovery after reload.

The suite targets current accessible names, existing square coordinates and player clock attributes. It deliberately does not introduce test-only endpoints or production UI hooks. The optional-chunk matcher targets the actual built `club-hub-*.js` filename; if bundling changes that name, update the matcher against the emitted manifest and retain the `blocked > 0` assertion so the test cannot silently pass without injecting failure.
