...
export function Club({ room, busy, online, offset, act, run }: {
    ...
    const { me } = room;
    const character = characterFor(me);
    useEffect(() => {
    ...
    function playerRow(id: PlayerId) {
        ...
        return <div className="player-row character-player" data-character={characterFor(id)?.key || 'guest'}>
            <CharacterPortrait player={id} name={who(id)}/><div className="player-info"><strong>{who(id)} {id === me && <small>YOU</small>}</strong><span>{color === 'w' ? 'White pieces' : 'Black pieces'}</span></div>
