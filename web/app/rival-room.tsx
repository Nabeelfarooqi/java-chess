...
export function Club({ room, busy, online, offset, act, run }: {
    ...
    function playerRow(id: PlayerId) { const color = g ? (g.white === id ? 'w' : 'b') : (id === me ? 'w' : 'b'); return <div className="player-row"><div className={`avatar ${id === me ? 'self' : ''}`}>{who(id).slice(0, 1).toUpperCase()}</div><div className="player-info"><strong>{who(id)} {id === me && <small>YOU</small>}</strong><span>{color === 'w' ? 'White pieces' : 'Black pieces'}</span></div><PlayerClock game={g} color={color} offset={offset} minutes={Number(minutes)}/></div>; }
    ...
    return <main className="club"><header className="club-header"><div className="club-identity"><div className="brand"><span className="brand-mark">♞</span> RIVAL ROOM <span className="private-badge"><LockKeyhole size={11}/> INVITE ONLY</span></div><p className="signed-in-player">Playing as <strong>{who(me)}</strong></p></div><div className="header-actions"><span className={`connection ${online ? '' : 'offline'}`}><i />{online ? 'Connected' : 'Reconnecting…'}</span><Button aria-label="Invite your rival" variant="outline" onClick={share}><Link2 size={15}/><span>{copied ? 'Copied' : 'Invite your rival'}</span></Button><Button aria-label="Room settings" size="icon" variant="ghost" onClick={() => { setName(who(me)); setSettings(true); }}><Settings2 /></Button></div></header>
 ...
 <footer><span>RIVAL ROOM · YOUR PRIVATE CLUB</span><span>One board. A lasting rivalry.</span></footer>
