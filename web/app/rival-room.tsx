...
import { ArrowRight, ArrowDownUp, Link2, LockKeyhole, LogOut, Settings2, Flag, Handshake, Download, Zap, Trophy, History, ChevronRight, CircleHelp } from 'lucide-react';
...
export default function RivalRoom() {
    const { room, busy, ready, error, online, live, latency, moveLatency, offset, act } = useRoom();
    ...
    return <>{room ? <Club key={room.me} room={room} busy={busy} online={online} live={live} latency={latency} moveLatency={moveLatency} offset={offset} act={act} run={run}/> : <Entry onEnter={pin => run('login', { pin })} busy={busy} error={error}/>}<Toaster theme="dark" richColors position="bottom-center"/></>;
}
export function Club({ room, busy, online, live, latency, moveLatency, offset, act, run }: {
    room: Room;
    busy: boolean;
    online: boolean;
    live: boolean;
 ...
 <Dialog open={boardHelp} onOpenChange={setBoardHelp}><DialogContent><DialogHeader><DialogTitle>Board controls</DialogTitle><DialogDescription>Drag a piece, or tap it and then its destination.</DialogDescription></DialogHeader><div className="board-help"><h3>Castling</h3><p>Move your king two squares toward a rook, or tap your king and then that rook. You can also drag the king onto the rook.</p><p>The path must be clear. Neither piece can have moved, and your king cannot castle out of, through, or into check.</p><h3>Premoves</h3><p>On your rival’s turn, choose your next move the same way. It plays only if it is legal when your turn arrives. Use Cancel below the board to clear it.</p></div></DialogContent></Dialog>
