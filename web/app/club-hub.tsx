...
export default function ClubHub({me,playing,onChallenge,onPractice}:{me:string;playing:boolean;onChallenge:(id:string)=>void;onPractice:()=>void}) {
    ...
    useEffect(()=>{
        ...
        async function load(){if(document.hidden)return;try{const r=await fetch('/api/room?club=1',{cache:'no-store',signal:controller.signal});if(!r.ok)throw Error('Could not load the club. Reopen this tab to try again.');const next=await r.json() as {players:Player[];leaders:Leader[]};if(!stopped){setData(next);setError('');}}catch(e){if(!stopped)setError((e as Error).message);}}
